    ...
    def proxy_url_for(self, port: int | str, path: str = "/", *, scheme: str = "https://") -> str:
        ...
        - **Pool / shared sandbox**: it cannot bind a TCP port (Landlock), so bind a
          **unix socket** at `$SBX_PROXY_DIR/<port>.sock` (the `SBX_PROXY_DIR` env var
          is set in every sandbox). E.g. `uvicorn app:app --uds $SBX_PROXY_DIR/8100.sock`.
        ...
        Returns:
            `str`: a URL like `https://<job_id>--49983.hf.jobs/v1/.../proxy/8100/ws` (or
            `wss://...` with `scheme="wss://"`).

        Example:
            ```python
            >>> url = sandbox.proxy_url_for(8100, "/ws", scheme="wss://")
            >>> import websockets
